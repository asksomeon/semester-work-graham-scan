...
// Добро пожаловать в наш замок в Сен-Мартен-де-Бельвиль...

// абсолютный путь до набора данных и папки проекта
static constexpr auto kDatasetPath = string_view{PROJECT_DATASET_DIR};

int main() {
  /* Так-то тестовые данные уже сгенерированы, но если нужны новые, то переходим в dataset, удаляем две папочки:
   * abs и ord. Переходим в  generate_csv_dataset.py, нажимаем на зеленую стрелочку. Оп, появились.
   * Если не нужны, то смотрим ниже.
   */
  const auto path = string(kDatasetPath);
  cout << "Path to the 'dataset/' folder: " << path << endl;

// Так как динамические массивы в плюсах больше похожи на... не будем о плохом. Вообщем в [] вставляем столько, сколько точек мы будем обрабатывать (цыферки в () в файликах,
// которые мы обрабатываем)
  ...
  // Чтение файла (надеюсь, тут все понятно)
  ...
  //эта хрень нужна, чтобы при повторном запуске можно было разделять старые и новые данные (6 часов работы подряд...)
  out<<"----------------------------------"<<endl;
  out.close();
  return 0;
}
